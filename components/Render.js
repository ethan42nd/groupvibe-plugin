...
const Render = {
    async render(path, params, cfg) {
        ...
        return e.runtime.render('groupvibe-plugin', path, params, {
            retType: cfg.retMsgId ? 'msgId' : 'default',
            beforeRender({ data }) {
                let pluginName = ''
                if (data.pluginName !== false) {
                    pluginName = ` & ${data.pluginName || 'groupvibe-plugin'}`
                    if (data.pluginVersion !== false) {
                        pluginName += `<span class="version">${currentVersion}`
                    }
                }
                let resPath = data.pluResPath
                const layoutPath = process.cwd() + '/plugins/groupvibe-plugin/resources/common/layout/'
                return {
                    ...data,
                    _res_path: resPath,
                    _mc_path: resPath,
                    ...
                    copyright: `Created By Yunzai-Bot${pluginName}</span>`,
                    pageGotoParams: {
