...
export class EmojiReaction extends plugin {
    ...
    async handleEmojiReaction(e) {
        ...
        const userEnabled = await this.getUserEmojiReactionEnabled(String(e.user_id), emojiConfig.globalEnabled || false)
        if (!userEnabled) {
        ...
        const now = Date.now()
        const cooldown = (emojiConfig.cooldown || 5) * 1000
        ...
        try {
            if (emojiConfig.useSameEmoji) {
                ...
                const reactionEmojiId = emojiConfig.emojiId || '74'
